...
public class MotivationPopulator {
	...
	public static void feedRefactoringsInstances(BigInteger refactoringsFlag, int systemsFlag, MotivationTestBuilder test)
	...
	private static void prepareFSERefactorings(MotivationTestBuilder test, BigInteger flag)
			throws JsonParseException, JsonMappingException, IOException {
	...
	public static String[] extractMotivations(List<Motivation> motivation) {
		String[] motivations = new String[motivation.size()];
		int counter = 0;
		for (Motivation m : motivation) {
				motivations[counter++] = m.label;
		}
		return motivations;
	}
	public static String[] extractRefactorings(List<Refactoring> refactoring) {
		/*
		 * int count = 0; for (Refactoring ref : refactoring) { if
		 * (ref.validation.contains("TP")) count++; }
		 */
	...
	protected static boolean isAdded(Refactoring refactoring, BigInteger flag) {
		try {
